...
| `npm test`      | ✅ 11 파일 / 206 passed + probe(skip)                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `npm run check` | ✅ 0 errors / 0 warnings (435 files)                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `npm run lint`  | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| git             | 원격 **github.com/pulunick/hcroi-simulator (public)**, 브랜치 **`product/commercial`** a2581d7 = main 0174892(공개판 겉면 정리, 세션 13) + `SITE_ENABLED=true` 한 줄. 이 브랜치의 state.md 는 공개판 진행 상황만                                                                                                                                                                                                                                                                                                   |
...
- **다음 세션 시작점**: 0. QA 재검증 이력 — 2차(7131193) 18/20 PASS + 신규 2건 → main 508280c 수정(시작 경로 회귀·PDF 인건비 열 기준·인원 증감 회색·다크 입력칸) → 1차(main) 전 항목 PASS → product **02a6ad7**(머지, 플래그 true) 공개판 분기 재확인 요청 → push(지시 시) → Vercel 배포
...
- QA 세션 재검증 스크립트를 **`tests/qa/` 로 편입·커밋**(main b900b07, 사용자 지시): `sweep·items·baseline·branch·dark·site·pdf.mjs` + `lib.mjs`, `npm run qa`(dev 서버 5173 + 로컬 Chrome 필요, playwright devDependency), README. 원본 이름: `v2-sweep.mjs`(7화면 스윕, env BASE·OUT·TAG·W) · `v2-items.mjs`(기간 추가·인원 0·오류 레코드·합산·삭제) · `v3-base.mjs`(기준선 3종) · `v3-main.mjs`(브랜치 분기·`?start=`·증감 색) · `v3-pdf.mjs`(PDF 카드, env PDF1·PDF2) · `v4.mjs`(공개판 분기·랜딩 CTA) · `v2-dark.mjs`. 실행은 playwright 가 있는 스크래치패드 `node_modules` 기준 `node <파일>`, `channel:'chrome'`.

### 2026-09-16 — 세션 13 (product): 공개판 겉면 정리 머지
