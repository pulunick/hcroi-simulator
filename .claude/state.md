...
> 2026-09-03 인사담당자 회신으로 재작성. 확정된 기준은 [brain.md §11](brain.md) 참조.

### A. 작고 확실한 것 (추가 비용 0, 먼저 처리)

1. **금액 천원 단위 표기** — `format.ts` 표시 계층만 변경(저장은 원 단위 유지). 엑셀 `NUM_FMT.won` 서식·헤더 단위 표기·user-guide 동시 갱신
2. **인건비 세부 4항목만 입력 허용** — 현재 엑셀 가져오기가 "세부 합계 ≠ 총액"을 **오류로 막아** 4항목만 채운 파일이 거부된다.
   합계 < 총액이면 경고로 낮추고 차액을 미분류로 두는 방식 검토. `fromRows.ts` + 테스트 + UI 세부 입력 패널
3. **임직원 수 산정 기준 옵션** — 기간 평균(FTE) 기본, 계약직·파견·등기임원 포함 토글. 화면 표기에 기준 명시

### B. 큰 것 (착수 순서 결정 필요)

4. **분기 단위 지원** — 필수 요구. `YearRecord` → 기간 레코드 확장이라 spec·엑셀 시트·차트·테스트 전면 파급. 단독으로 착수할 것
5. **부서별 HCROI** — 배부율 α = 인건비 비중(확정), 단위 본부–팀. 부서 마스터 입력 화면 필요
6. **결산서 PDF 파싱** — 원본 PDF 수령이 확인되어 **브라우저 텍스트 추출로 가능한지 먼저 검증**(실제 파일 1부 필요).
   가능하면 서버·비용 없이 진행, 불가하면 비용 안내 후 재논의
7. **동종업계 평균 비교** — 6번 파서 위에 얹는 기능. 비교 대상 회사를 어디서 몇 개 구하는지 확인 후

### 완료

- ~~md 포맷 정리~~ / ~~첫 커밋~~ / ~~화면 육안 점검·이슈 9건~~ / ~~엑셀 내보내기·가져오기~~ / ~~원격 저장소~~ / ~~등급 3단계~~ / ~~대시보드 제목·조직 정보 시트~~
- **Vercel 배포** — 사용자가 대시보드에서 pulunick/hcroi-simulator import (SvelteKit 기본 설정). 단독 사용이 확정돼 링크 공유는 불필요하지만 본인 접근용으로는 여전히 유용

## 막힌 것 / 사용자 결정 대기

> 2026-09-03 회신으로 기존 3건(DB 도입 · 배부율 α · 분석 단위)은 모두 해소됨. 아래는 새로 생긴 확인 사항.

- **부서별 인건비·인원이 실제로 본부–팀 단위로 관리되고 있는지** — 문항 9에서 "필요하다"만 답하고 관리 여부는 답이 없었다. 없으면 부서별 화면을 만들어도 넣을 데이터가 없다
- **동종업계 결산 보고서를 어디서, 몇 개 사 분량으로 구하는지** — 공시 자료인지 협회 자료인지에 따라 파싱 난이도와 법적 검토가 달라짐
- **천원 단위가 표시만인지 입력도 천원으로 받는지** — 입력까지 바꾸면 기존 저장 데이터·엑셀 파일과 단위가 어긋난다
- **분기 데이터도 3개년치가 있는지** — 문항 7의 "3개년"이 연간 기준인지 분기 기준인지 불명확
