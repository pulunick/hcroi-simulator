...
## 현재 상태 (2026-09-08 세션 5 진행 중)

| 항목            | 상태                                                                                                         |
| --------------- | ------------------------------------------------------------------------------------------------------------ |
| 단계            | 프로토타입 **기능 완료** + 확장 지침 1차 반영. **Vercel 배포 대기**(사용자가 import) → 인사담당자 시연 2단계 |
| `npm test`      | ✅ 4 파일 / 61 테스트 (formulas · scenario · insights · excel)                                               |
...
2. ~~금액 단위 표기~~ → **표시 단위 옵션**으로 구현 (자동·원·천원·백만원·억원, 기본 천원)

### 다음

3. **임직원 수 산정 기준 옵션** — 기간 평균(FTE) 기본, 계약직·파견·등기임원 포함 토글. 화면 표기에 기준 명시. (작은 것)
4. **분기 단위 지원** — 필수 요구. `YearRecord` → 기간 레코드(`periodType: 'Y' | 'H' | 'Q'`) 확장이라
   spec·엑셀 시트·차트·테스트 전면 파급. **단독으로 착수할 것.** 분기 실적이 없는 구간은 반기 값을 반기 레코드로 그대로 저장(분기로 쪼개 추정 금지)
