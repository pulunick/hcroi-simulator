...
- **회사와 아무 연관 없는 개인 프로젝트다(2026-09-10 확인).** 인사담당자의 개인 요구에서 시작했고, 담당자에게 수익화 허락을 받았으며 개발자 개인이 진행한다. 회사 승인·규정 확인은 필요 없다. 위생 규칙만 유지(push 는 `pulunick` 계정으로, 실제 수치 미커밋). 허락은 문서로 남긴다(기획서 §12).
- 수익 전략은 **무료 공개(H1) → 결과물 과금(H2)** 순서로 검증, 파트너(H3) 접촉 병행(기획서 §11). 계산 기능은 끝까지 무료. 결제·라이선스 코드와 저장소 private 전환은 H2 로 갈 때 다시 판단 — 그 전까지 이 브랜치에는 문서·디자인·무료 기능만. 제품명에 "Martin" 은 쓰지 않는다.
