...
2. `.Codex/brain.md` — 이번 세션에 **결정**된 규칙·가정·범위가 있으면 해당 § 갱신 (진행 상황은 넣지 않는다)
...
7. `AGENTS.md` — 새 절대 규칙·명령·파일 위치가 생겼으면 한 줄 추가
...
- 커밋 메시지: `type(scope): 요약` + 본문 bullet, 끝에 `Co-Authored-By: Codex Fable 5.1 <noreply@anthropic.com>`
